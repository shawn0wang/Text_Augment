...
class Seq2SeqSimFunc(BaseFunc, AutoRegressiveDecoder):
    def __init__(self, config):
        ...
        if model_name.split("_")[0] == "simbert":
            model_type = "bert"
        else:
            model_type = "roformer"
        model_config = seq2seq_model_config[model_name]
        model_path = f"function/model/{model_config['model_dir']}"
        if not os.path.exists(model_path):
            ...
            with zipfile.ZipFile(f"{model_path}.zip", "r") as zip_ref:
                zip_ref.extractall(path="function/model")
        ...
        self.threshold = config.seq2seq_sim_func.threshold
        AutoRegressiveDecoder.__init__(self, self.start_id, self.end_id, self.max_len)

    @staticmethod
    def bar_progress(current, total, width=80):
        progress_message = "Seq2SeqSimFunc Model Downloading: %d%% [%d / %d] bytes" \
    ...
    def predict(self, inputs, output_ids, states):
        ...
        return self.last_token(self.seq2seq).predict([token_ids, segment_ids])

    def generate(self, text, n=1):
        token_ids, segment_ids = self.tokenizer.encode(text, maxlen=self.maxlen)
        output_ids = self.random_sample([token_ids, segment_ids], n, topp=0.95)  # 基于随机采样
    ...
    def process(self, sentence):
        ...
        return [r[i + 1] for i in argsort[::-1][:self.augment_num] if scores[i] >= self.threshold]
